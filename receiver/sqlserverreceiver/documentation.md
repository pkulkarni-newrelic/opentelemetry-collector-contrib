...
Number of database role memberships per role.

This metric is only available when the receiver is configured to directly connect to SQL Server. Tracks database role membership assignments per role. The db.namespace attribute contains the database name and the role attribute contains the role name. Critical for monitoring privilege escalation and compliance. Available on SQL Server 2012+.
...
Number of server role memberships per role.

This metric is only available when the receiver is configured to directly connect to SQL Server. Tracks server role membership assignments per role. The role attribute contains the server role name. Critical for monitoring high-privilege access and compliance. Available on SQL Server 2012+.
