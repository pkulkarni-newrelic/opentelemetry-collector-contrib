...
INNER JOIN sys.server_principals sp WITH (NOLOCK) ON srm.role_principal_id = sp.principal_id
GROUP BY sp.name
{filter_instance_name};
`
...
func getSQLServerSecurityRoleMembersQuery(instanceName string) string {
	if instanceName != "" {
		whereClause := fmt.Sprintf("\nHAVING @@SERVERNAME = '%s'", instanceName)
		r := strings.NewReplacer("{filter_instance_name}", whereClause)
...
IF @EngineEdition = 5 BEGIN -- Azure SQL Database
	...
	INNER JOIN sys.database_principals dp WITH (NOLOCK) ON drm.role_principal_id = dp.principal_id
	GROUP BY dp.name
	{filter_instance_name};
END
ELSE BEGIN
	-- For on-premises, we need to iterate through databases
	-- This query returns role membership counts per role per database
	...
	SELECT
		...
		dp.name AS [role_name],
		COUNT(*) AS [role_member_count]
	FROM [' + DB_NAME(database_id) + '].sys.database_role_members drm WITH (NOLOCK)
	INNER JOIN [' + DB_NAME(database_id) + '].sys.database_principals dp WITH (NOLOCK) ON drm.role_principal_id = dp.principal_id
	GROUP BY dp.name
	UNION ALL'
	FROM sys.databases
